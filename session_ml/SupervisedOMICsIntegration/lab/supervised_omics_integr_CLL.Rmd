...
knitr::opts_knit$set(root.dir="~/github/workshop_omics_integration/session_ml/SupervisedOMICsIntegration/lab")
...
expr <- as.data.frame(t(CLL_data$mRNA))  # read.delim("data/CLL_mRNA.txt", header = TRUE, sep="\t")))
for(i in 1:ncol(expr)){expr[,i][is.na(expr[,i])]<-median(expr[,i],na.rm=TRUE)}
expr[1:5,1:5]

mut <- as.data.frame(t(CLL_data$Mutations))  # read.delim("data/CLL_Mutations.txt", header = TRUE, sep="\t")))
for(i in 1:ncol(mut)){mut[,i][is.na(mut[,i])]<-median(mut[,i],na.rm=TRUE)}
mut[1:5,1:5]

meth <- as.data.frame(t(CLL_data$Methylation))  # read.delim("data/CLL_Methylation.txt", header = TRUE, sep="\t")))
for(i in 1:ncol(meth)){meth[,i][is.na(meth[,i])]<-median(meth[,i],na.rm=TRUE)}
meth[1:5,1:5]

drug <- as.data.frame(t(CLL_data$Drugs))  # read.delim("data/CLL_Drugs.txt", header = TRUE, sep="\t")))
...
```{r,fig.width=10,fig.height=8}
data("CLL_covariates")
phen <- CLL_covariates  # read.delim("data/CLL_Covariates.txt", header = TRUE, sep="\t")
head(phen)
...
# https://github.com/LeandroRitter/workshop_omicsint_ISMBECCB/blob/main/session_ml/SupervisedOMICsIntegration/ENSEMBLE_TO_GENE_SYMBOL_COORD.txt
ens2gs<-read.delim("~/github/workshop_omics_integration/session_ml/SupervisedOMICsIntegration/lab/data/ENSEMBLE_TO_GENE_SYMBOL_COORD.txt",header=TRUE,sep="\t")
...
perf.diablo = perf(splsda.res, validation = 'Mfold', folds = 2, nrepeat = 5, progressBar=FALSE, cpus=8)
perf.diablo
